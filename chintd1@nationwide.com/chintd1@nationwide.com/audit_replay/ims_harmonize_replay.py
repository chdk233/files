...
if (ims_trip_summary_missing_trips.count()>0):
  ...
  print("trips_missing")
  ims_trip_summary_missing_trips.withColumn("etl_load_date",current_date()).withColumn("etl_load_time",current_timestamp()).write.format("delta").mode("append").option("mergeSchema","True").saveAsTable(f"dhf_iot_ims_stageraw_{environment}.trip_summary_missing_trips")
  source_system=source_system+([row[0] for row in (spark.sql(f"select distinct sourcesystem from  dhf_iot_ims_stageraw_{environment}.trip_summary_missing_trips where etl_load_date=current_date ")).collect()])
...
(select * from dhf_iot_ims_raw_{environment}.telemetrypoints where load_date>=current_date-{days_end_count} and load_date<=current_date-{days_start_count}) a left anti join
(select * from dhf_iot_harmonized_{environment}.trip_point where load_dt>=current_date-{days_end_count}-6 and src_sys_cd in ("IMS_SM_5X","IMS_SR_4X","IMS_SR_5X")) b 
...
(select * from dhf_iot_ims_raw_{environment}.telemetryevents where load_date>=current_date-{days_end_count} and load_date<=current_date-{days_start_count}) a left anti join
(select * from dhf_iot_harmonized_{environment}.trip_event where load_dt>=current_date-{days_end_count}-6 and src_sys_cd in ("IMS_SM_5X","IMS_SR_4X","IMS_SR_5X")) b 
on   a.tripsummaryid=b.trip_smry_key  and a.sourcesystem=b.src_sys_cd where tripsummaryid is not null""")

chlg_query=f"""select distinct cast(row_number() over(order by NULL) +5000+ coalesce((select max(TRIP_EVENT_ID) from dhf_iot_harmonized_{environment}.trip_event),0) as BIGINT) as TRIP_EVENT_ID, coalesce(tdr.db_load_time, to_timestamp('9999-12-31')) AS ETL_ROW_EFF_DTS, current_timestamp() as ETL_LAST_UPDT_DTS, cast(tdr.acceleration as decimal(18,5)) as ACLRTN_RT, cast(tdr.secondsOfDriving as decimal(15,6)) as DRVNG_SC_QTY, cast(tdr.telemetryEventSeverityLevel as STRING) as EVNT_SVRTY_CD, case tdr.sourcesystem when ('IMS_SR_4X') THEN coalesce(b.Description, 'NOKEY') else coalesce(tdr.telemetryEventType, 'NOKEY') end EVNT_TP_CD, cast(tdr.headingDegrees as decimal(15,6)) as HEADNG_DEG_QTY, cast(tdr.speed as decimal(18,10)) as SPD_RT, tdr.tripSummaryId AS TRIP_SMRY_KEY, cast(tdr.telemetryEvents__avgSpeed as decimal(18,10)) as AVG_SPD_RT, coalesce(cast(tdr.utcDateTime as TIMESTAMP), to_timestamp('9999-12-31')) as UTC_TS, cast(tdr.Event_Reference_Value as decimal(35,15)) as EVT_RFRNC_VAL, cast(tdr.Event_Timezone_Offset as int) as EVT_TMZN_OFFST_NUM, cast(tdr.degreesLatitude as decimal(18,10)) as LAT_NB, cast(tdr.degreesLongitude as decimal(18,10)) as LNGTD_NB, coalesce(tdr.load_date, to_date('9999-12-31')) as LOAD_DT, coalesce(tdr.sourcesystem, 'NOKEY') AS SRC_SYS_CD, coalesce(tdr.load_hour, to_timestamp('9999-12-31')) as LOAD_HR_TS , null as EVNT_STRT_TS, null as EVNT_END_TS, null as EVNT_DRTN_QTY from  dhf_iot_ims_stageraw_{environment}.trip_event_missing_trips tdr left join dhf_iot_harmonized_{environment}.event_code b on tdr.sourcesystem=b.source and tdr.telemetryEventType=b.code where etl_load_date=current_date"""


if (ims_trip_event_missing_trips.count()>0):
  tables_list= tables_list + ['Trip_event']
  print("trips_missing")
  ims_trip_event_missing_trips.withColumn("etl_load_date",current_date()).withColumn("etl_load_time",current_timestamp()).write.format("delta").mode("append").option("mergeSchema","True").saveAsTable(f"dhf_iot_ims_stageraw_{environment}.trip_event_missing_trips")
  source_system=source_system+([row[0] for row in (spark.sql(f"select distinct sourcesystem from  dhf_iot_ims_stageraw_{environment}.trip_event_missing_trips where etl_load_date=current_date ")).collect()])
  spark.sql(f"{chlg_query}").write.format("delta").mode("append").saveAsTable(f"dhf_iot_harmonized_{environment}.trip_event_chlg")
...
(select * from dhf_iot_ims_raw_{environment}.nontripevent where load_date>=current_date-{days_end_count} and load_date<=current_date-{days_start_count}) a left anti join
(select * from dhf_iot_harmonized_{environment}.non_trip_event where load_dt>=current_date-{days_end_count}-6 and src_sys_cd in ("IMS_SM_5X","IMS_SR_4X","IMS_SR_5X")) b 
on   a.telemetrySetId=b.NON_TRIP_EVNT_KEY  and a.sourcesystem=b.src_sys_cd where telemetrySetId is not null""")

chlg_query=f"""select distinct cast(row_number() over(order by NULL) +2000+ coalesce((select max(NON_TRIP_EVENT_ID) from dhf_iot_harmonized_{environment}.non_trip_event),0) as BIGINT) as NON_TRIP_EVENT_ID, coalesce(tdr.telemetrySetId, 'NOKEY') as NON_TRIP_EVNT_KEY, coalesce(tdr.db_load_time, to_timestamp('9999-12-31')) AS ETL_ROW_EFF_DTS, current_timestamp() as ETL_LAST_UPDT_DTS, case when coalesce(tdr.deviceIdentifier,tdr.deviceSerialNumber,crc32(tdr.detectedvin)) is null then 'NOKEY' ELSE coalesce(tdr.deviceIdentifier,tdr.deviceSerialNumber,crc32(tdr.detectedvin)) END DEVC_KEY, cast(tdr.acceleration as decimal(18,5)) as ACLRTN_RT, cast(tdr.telemetryEventSeverityLevel as STRING) as EVNT_SVRTY_CD, case tdr.sourcesystem when ('IMS_SR_4X') THEN coalesce(b.Description, 'NOKEY') else coalesce(tdr.telemetryEventType, 'NOKEY') end EVNT_TP_CD, cast(tdr.headingDegrees as decimal(15,6)) as HEADNG_DEG_QTY, cast(tdr.speed as decimal(18,5)) as SPD_MPH_RT, coalesce(cast(tdr.utcDateTime as TIMESTAMP), to_timestamp('9999-12-31')) as UTC_TS, coalesce(tdr.enrolledVin,tdr.detectedVin) AS VEH_KEY, cast(tdr.Event_Reference_Value as decimal(35,15)) as EVT_RFRNC_VAL, cast(tdr.Event_Timezone_Offset as INT) as EVT_TMZN_OFFST_NUM, cast(tdr.degreesLatitude as decimal(18,10)) as LAT_NB, cast(tdr.degreesLongitude as decimal(18,10)) as LNGTD_NB, coalesce(tdr.load_date, to_date('9999-12-31')) as LOAD_DT, coalesce(tdr.sourcesystem, 'NOKEY') AS SRC_SYS_CD, coalesce(tdr.load_hour, to_timestamp('9999-12-31')) as LOAD_HR_TS from dhf_iot_ims_stageraw_{environment}.non_trip_event_missing_trips tdr left join dhf_iot_harmonized_{environment}.event_code b on tdr.sourcesystem=b.source and tdr.telemetryEventType=b.code where etl_load_date=current_date"""


if (ims_non_trip_event_missing_trips.count()>0):
  tables_list= tables_list + ['Non_trip_event']
  print("trips_missing")
  ims_non_trip_event_missing_trips.withColumn("etl_load_date",current_date()).withColumn("etl_load_time",current_timestamp()).write.format("delta").mode("append").option("mergeSchema","True").saveAsTable(f"dhf_iot_ims_stageraw_{environment}.non_trip_event_missing_trips")
  source_system=source_system+([row[0] for row in (spark.sql(f"select distinct sourcesystem from  dhf_iot_ims_stageraw_{environment}.non_trip_event_missing_trips where etl_load_date=current_date ")).collect()])
  
  spark.sql(f"{chlg_query}").write.format("delta").mode("append").saveAsTable(f"dhf_iot_harmonized_{environment}.non_trip_event_chlg")
...
(select distinct enrolledVin,detectedVin,load_date,load_hour,db_load_date,db_load_time,sourcesystem from dhf_iot_ims_raw_{environment}.tripsummary where load_date>=current_date-{days_end_count} and load_date<=current_date-{days_start_count}) a left anti join
(select * from dhf_iot_harmonized_{environment}.vehicle where src_sys_cd in ("IMS_SM_5X","IMS_SR_4X","IMS_SR_5X")) b 
...
(select distinct enrolledVin,detectedVin,load_date,load_hour,db_load_date,db_load_time,sourcesystem from dhf_iot_ims_raw_{environment}.nontripevent where load_date>=current_date-{days_end_count} and load_date<=current_date-{days_start_count}) a left anti join
(select * from dhf_iot_harmonized_{environment}.vehicle where  src_sys_cd in ("IMS_SM_5X","IMS_SR_4X","IMS_SR_5X")) b 
on   a.enrolledVin=b.enrld_vin_nb  and a.sourcesystem=b.src_sys_cd where enrolledVin is not null""")

ims_vehicle_missing_vins=ims_vehicle_trip_summary_missing_trips.union(ims_non_trip_event_missing_trips).distinct()

chlg_query=f"""select distinct cast(row_number() over(order by NULL) +700+ coalesce((select max(VEHICLE_ID) from dhf_iot_harmonized_{environment}.vehicle),0) as BIGINT) as VEHICLE_ID, COALESCE(enrolledVin,detectedVin,'NOKEY') AS VEH_KEY, coalesce(db_load_time, to_timestamp('9999-12-31')) AS ETL_ROW_EFF_DTS, current_timestamp() as ETL_LAST_UPDT_DTS, COALESCE(detectedVin,enrolledVin) as DTCTD_VIN_NB, COALESCE(enrolledVin,detectedVin,'NOKEY') as ENRLD_VIN_NB, coalesce(load_date, to_date('9999-12-31')) as LOAD_DT, coalesce(SourceSystem,'NOKEY') AS SRC_SYS_CD, coalesce(load_hour, to_timestamp('9999-12-31')) as LOAD_HR_TS from dhf_iot_ims_stageraw_{environment}.vehicle_missing_vins where etl_load_date=current_date"""


if (ims_vehicle_missing_vins.count()>0):
  tables_list= tables_list + ['Vehicle']
  print("trips_missing")
  ims_vehicle_missing_vins.withColumn("etl_load_date",current_date()).withColumn("etl_load_time",current_timestamp()).write.format("delta").mode("append").saveAsTable(f"dhf_iot_ims_stageraw_{environment}.vehicle_missing_vins")
  source_system=source_system+([row[0] for row in (spark.sql(f"select distinct sourcesystem from  dhf_iot_ims_stageraw_{environment}.vehicle_missing_vins where etl_load_date=current_date ")).collect()])
...
(select distinct deviceSerialNumber,deviceIdentifier,load_date,load_hour,db_load_date,db_load_time,sourcesystem,detectedvin,deviceIdentifierType,deviceType from dhf_iot_ims_raw_{environment}.tripsummary where load_date>=current_date-{days_end_count} and load_date<=current_date-{days_start_count}) a left anti join
(select * from dhf_iot_harmonized_{environment}.device where   src_sys_cd in ("IMS_SM_5X","IMS_SR_4X","IMS_SR_5X")) b 
...
(select * from dhf_iot_ims_raw_{environment}.scoring where load_date>=current_date-{days_end_count} and load_date<=current_date-{days_start_count}) a left anti join
(select * from dhf_iot_harmonized_{environment}.scoring where load_dt>=current_date-{days_end_count}-6 and src_sys_cd in ("IMS_SM_5X","IMS_SR_5X")) b 
on   a.tripsummaryid=b.trip_smry_key  and a.sourcesystem=b.src_sys_cd where tripsummaryid is not null""")

chlg_query=f"""select distinct cast(row_number() over(order by NULL) +3000+ coalesce((select max(SCORING_ID) from dhf_iot_harmonized_{environment}.SCORING),0) as BIGINT) as SCORING_ID, coalesce(db_load_time, to_timestamp('9999-12-31')) AS ETL_ROW_EFF_DTS, tripSummaryId AS TRIP_SMRY_KEY, current_timestamp() as ETL_LAST_UPDT_DTS, scoreAlgorithmProvider as SCR_ALGRM_PRVDR_NM, scoreUnit as SCR_UNIT_CD, cast(overallScore as decimal(18,10))as OVRL_SCR_QTY, coalesce(component, 'NOKEY') as INDV_CMPNT_SET_TP_VAL, cast(coalesce(componentScore ,0) as decimal(18,10)) as INDV_CMPNT_SET_SCR_QTY, coalesce(load_date, to_date('9999-12-31')) as LOAD_DT, coalesce(SourceSystem, 'NOKEY') as SRC_SYS_CD, coalesce(load_hour, to_timestamp('9999-12-31')) as LOAD_HR_TS from dhf_iot_ims_stageraw_{environment}.scoring_missing_trips where etl_load_date=current_date"""


if (ims_scoring_missing_trips.count()>0):
  tables_list= tables_list + ['Scoring']
  print("trips_missing")
  ims_scoring_missing_trips.withColumn("etl_load_date",current_date()).withColumn("etl_load_time",current_timestamp()).write.format("delta").mode("append").option("mergeSchema","True").saveAsTable(f"dhf_iot_ims_stageraw_{environment}.scoring_missing_trips")
  source_system=source_system+([row[0] for row in (spark.sql(f"select distinct sourcesystem from  dhf_iot_ims_stageraw_{environment}.scoring_missing_trips where etl_load_date=current_date ")).collect()])
  
  spark.sql(f"{chlg_query}").write.format("delta").mode("append").saveAsTable(f"dhf_iot_harmonized_{environment}.scoring_chlg")
...
(select * from dhf_iot_ims_raw_{environment}.histogramscoringintervals where load_date>=current_date-{days_end_count} and load_date<=current_date-{days_start_count}) a left anti join
(select * from dhf_iot_harmonized_{environment}.histogram_scoring_interval where load_dt>=current_date-{days_end_count}-6 and src_sys_cd in ("IMS_SM_5X","IMS_SR_5X")) b 
on   a.tripsummaryid=b.trip_smry_key  and a.sourcesystem=b.src_sys_cd where tripsummaryid is not null""")

chlg_query=f"""select distinct cast(row_number() over(order by NULL) +3000+ coalesce((select max(HISTOGRAM_SCORING_INTERVAL_ID) from dhf_iot_harmonized_{environment}.HISTOGRAM_SCORING_INTERVAL),0) as BIGINT) as HISTOGRAM_SCORING_INTERVAL_ID, coalesce(db_load_time, to_timestamp('9999-12-31')) AS ETL_ROW_EFF_DTS, current_timestamp() as ETL_LAST_UPDT_DTS, tripSummaryId AS TRIP_SMRY_KEY, coalesce(scoringComponent, 'NOKEY') as SCRG_CMPNT_TP_CD, scoringSubComponent as SCRG_SUB_CMPNT_TP_CD, roadType as ROAD_TP_DSC, cast(thresholdLowerBound as decimal(18,10)) as LOWER_BND_THRSHLD_QTY, cast(thresholdUpperBound as decimal(18,10)) as UPPER_BND_THRSHLD_QTY, scoringComponentUnit as SCRG_CMPNT_UNIT_CD, cast(occurrences as int) as OCRNC_CT, occurrenceUnit as OCRNC_UNIT_CD, coalesce(load_date, to_date('9999-12-31')) as LOAD_DT, coalesce(SourceSystem, 'NOKEY') as SRC_SYS_CD, coalesce(load_hour, to_timestamp('9999-12-31')) as LOAD_HR_TS from dhf_iot_ims_stageraw_{environment}.histogramscoringintervals_missing_trips where etl_load_date=current_date """


if (ims_histogramscoringintervals_missing_trips.count()>0):
  tables_list= tables_list + ['Histogram_scoring_interval']
  print("trips_missing")
  ims_histogramscoringintervals_missing_trips.withColumn("etl_load_date",current_date()).withColumn("etl_load_time",current_timestamp()).write.format("delta").mode("append").option("mergeSchema","True").saveAsTable(f"dhf_iot_ims_stageraw_{environment}.histogramscoringintervals_missing_trips")
  source_system=source_system+([row[0] for row in (spark.sql(f"select distinct sourcesystem from  dhf_iot_ims_stageraw_{environment}.histogramscoringintervals_missing_trips where etl_load_date=current_date ")).collect()])
  
  spark.sql(f"{chlg_query}").write.format("delta").mode("append").saveAsTable(f"dhf_iot_harmonized_{environment}.histogram_scoring_interval_chlg")
...
if mail_flag=='true':
  if (len(tables_list)>0) & (len(source_system)>0):
    message = f"Hi team,\n\nFew trips are not processed into harmonized, Data replay completed for missing trips  for the following tables: \n{tables_list}  \n\nSource_system: \n{source_system} \n\nfor  run date: \n{audit_check_date}    \n\nThanks,\nP&C Data Management "
    subject = f"IMS Harmonize Audit check failure: Few trips are not processed for the date  {audit_check_date}"
    send_email(email_from, email_to, subject, message)
  else:
