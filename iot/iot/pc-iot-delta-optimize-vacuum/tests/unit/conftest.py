...
def dbutils_fixture() -> Iterator[None]:
    ...
    with patch("pc_iot_delta_optimize_vacuum.common.get_dbutils", lambda _: DBUtilsFixture()):
        yield
    logging.info("Test session finished, patching completed")

@pytest.fixture(scope="session", autouse=True)
def constants(spark: SparkSession) -> dict:
    
    return \
        {'data_path' : spark.conf.get("spark.hive.metastore.warehouse.dir")[7:] }