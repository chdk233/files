...
```bash
conda create -n pc_iot_delta_optimize_vacuum python=3.9
conda activate pc_iot_delta_optimize_vacuum
```
...
```
# Runs coverage tests using pytest
pytest tests/unit --cov

# Runs coverage tests using pytest and generates an HTML Report in a dir labeled htmlcov
pytest tests/unit --cov html --cov-report=html

# Run coverage tests using coverage for three report types
coverage run -m pytest --junitxml=pytest-report.xml && coverage xml -o coverage.xml && coverage html
```
...
```
This command will create your personal repository under `/Repos/<username>/pc_iot_delta_optimize_vacuum`.
...
- To trigger the release pipeline, get the current version from the `pc_iot_delta_optimize_vacuum/__init__.py` file and tag the current code version:
```
