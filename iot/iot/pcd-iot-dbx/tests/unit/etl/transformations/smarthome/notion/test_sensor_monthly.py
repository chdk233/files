...
def sensor_monthly(spark: SparkSession):
    ...
    yield {'path' : sensor_monthly_path,
...
def test_sensor_monthly(spark,sensor_monthly: str):
    ...
    json_path = sensor_monthly.get('path')

    _metadata_field = sensor_monthly.get('_metadata_field')

    sensor_monthly_df = spark.read.json(json_path,
                                        schema=sensor_monthly_schema.add(_metadata_field))
