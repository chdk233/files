...
def test_event_weekly(spark,event_weekly: str):
    ...
    json_path = event_weekly.get('path')

    _metadata_field = event_weekly.get('_metadata_field')

    event_weekly_df = spark.read.json(json_path,
                                      schema=events_weekly_schema.add(_metadata_field))
